    ...
    public static class SqlMapperExtensions
    {
        ...
        public static IEnumerable<PropertyInfo> KeyPropertiesCache(Type type)
        {
            ...
            if (keyProperties.Count == 0)
            {
                ...
                var idProp2 = allProperties.Where(p => p.Name.ToLower() == "row_id").FirstOrDefault();
                if (idProp2 != null)
                {
                    keyProperties.Add(idProp2);
                }
            }
        ...
        //public static IEnumerable<T> GetList<T>(this IDbConnection connection, string sql, IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        //{
        //    return connection.Query<T>(sql, transaction: transaction, commandTimeout: commandTimeout);
        //}

        public static IEnumerable<T> GetList<T>(this IDbConnection connection, string sql, dynamic param = null, IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            return connection.Query<T>(sql, param as object, transaction: transaction, commandTimeout: commandTimeout);
        }
        ...
        public static long Insert<T>(this IDbConnection connection, T entityToInsert, IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            ...
            string spChar1 = "[", spChar2 = "]";
            if (connType.Name.ToLower() == "mysqlconnection")
            {
                spChar1 = "`";
                spChar2 = "`";
            }
            ...
            for (var i = 0; i < allPropertiesExceptKeyAndComputed.Count(); i++)
            {
                ...
                //MySql通过``来修改关键字同名列；SqlServer 用"[" 分割
                sbColumnList.AppendFormat("{0}{1}{2}", spChar1, property.Name, spChar2);
...
public class SqlServerAdapter : ISqlAdapter
{
    public int Insert(IDbConnection connection, IDbTransaction transaction, int? commandTimeout, String tableName, string columnList, string parameterList, IEnumerable<PropertyInfo> keyProperties, object entityToInsert)
    {
        ...
        var r = connection.Query("SELECT IDENT_CURRENT('" + tableName + "') Row_ID", transaction: transaction, commandTimeout: commandTimeout);
        int id = (int)r.First().Row_ID;
        if (keyProperties.Any())
...
public class MySQLAdapter : ISqlAdapter
{
    public int Insert(IDbConnection connection, IDbTransaction transaction, int? commandTimeout, String tableName, string columnList, string parameterList, IEnumerable<PropertyInfo> keyProperties, object entityToInsert)
    {
        ...
        var r = connection.Query("SELECT LAST_INSERT_ID() Row_ID", transaction: transaction, commandTimeout: commandTimeout);
        int id = (int)r.First().Row_ID;
        if (keyProperties.Any())
            keyProperties.First().SetValue(entityToInsert, id, null);
        return id;
